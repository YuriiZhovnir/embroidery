...
import kotlinx.android.synthetic.main.item_color_pdf.view.*


class ColorPdfAdapter (val patternId:Int) : RecyclerView.Adapter<ColorPdfAdapter.ViewHolder>() {
    private var realm = Realm.getDefaultInstance()
    private val cells = realm?.where(Pattern::class.java)?.equalTo("id", patternId)?.findFirst()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
                .inflate(R.layout.item_color_pdf, parent, false)
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var colorName = itemView.colorNameP
        var counter = itemView.counterP
        var colorInList = itemView.colorP
    }
}